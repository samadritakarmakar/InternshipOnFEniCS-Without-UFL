...
int main()
{
  ...
  auto SncF = std::make_shared<const fenicssolid::SinfoniettaClassica>(E, nu, beta, phiDegree, betaP, varKappa, Pc, varP, Pdash0);
...
//ADDED BY SAM For stress
  ...
  auto aStrss =std::make_shared<Plas3D::Form_aStrss2>(V5, V5);
  auto LStress = std::make_shared<Plas3D::Form_L_Strss2>(V5);
  LStress->u2 = u;
  ...
  auto V6=std::make_shared<Plas3D::CoefficientSpace_f_zero>(mesh);
  auto Pdash= std::make_shared<Function>(V6);
  auto aPdash = std::make_shared<Plas3D::Form_aPdash>(V6,V6);
  auto LPdash =std::make_shared<Plas3D::Form_LPdash>(V6);
  LPdash->u2 = u;
  LPdash->eps_p=constitutive_update->eps_p();
  LPdash->f_zero=std::make_shared<Constant>(0);

  auto Qdash= std::make_shared<Function>(V6);
  auto aQdash = std::make_shared<Plas3D::Form_aQdash>(V6,V6);
  auto LQdash =std::make_shared<Plas3D::Form_LQdash>(V6);
  LQdash->u2 = u;
  ...
  auto PdashForm = std::make_shared<Plas3D::Form_PdashForm>(mesh);
  PdashForm->u2 = u;
  ...
  auto QdashForm = std::make_shared<Plas3D::Form_QdashForm>(mesh);
  QdashForm->u2 = u;
  ...
  auto aEps_vForm=std::make_shared<Plas3D::Form_aEps_v>(V6,V6);
  auto LEps_vForm=std::make_shared<Plas3D::Form_LEps_v>(V6);
  LEps_vForm->u2=u;
  LEps_vForm->f_zero=ConstZeroScalar;
  auto Eps_vForm = std::make_shared<Plas3D::Form_Eps_vForm>(mesh);
  Eps_vForm->u2= u;
  ...
  auto aEpsP_vForm=std::make_shared<Plas3D::Form_aEpsP_v>(V6, V6);
  auto LEpsP_vForm=std::make_shared<Plas3D::Form_LEpsP_v>(V6);
  LEpsP_vForm->eps_p=constitutive_update->eps_p();
  LEpsP_vForm->f_zero=ConstZeroScalar;
  auto EpsP_vForm = std::make_shared<Plas3D::Form_EpsP_vForm>(mesh);
  ...
  auto aEps_zz_Form = std::make_shared<Plas3D::Form_aEps_zz>(V6, V6);
  auto LEps_zz_Form = std::make_shared<Plas3D::Form_LEps_zz>(V6);
  LEps_zz_Form->u2=u;
  LEps_zz_Form->f_zero=ConstZeroScalar;
  auto Eps_zz = std::make_shared<Plas3D::Form_Eps_zzForm>(mesh);
  Eps_zz->u2=u;
  ...
  auto aEps_devForm=std::make_shared<Plas3D::Form_aEps_dev>(V6, V6);
  auto LEps_devForm=std::make_shared<Plas3D::Form_LEps_dev>(V6);
  LEps_devForm->u2=u;
  LEps_devForm->f_zero=ConstZeroScalar;
  auto Eps_devForm = std::make_shared<Plas3D::Form_Eps_devForm>(mesh);
  Eps_devForm->u2=u;
  ...
  auto aEpsP_devForm=std::make_shared<Plas3D::Form_aEpsP_dev>(V6, V6);
  auto LEpsP_devForm=std::make_shared<Plas3D::Form_LEpsP_dev>(V6);
  LEpsP_devForm->eps_p=constitutive_update->eps_p();
  LEpsP_devForm->f_zero=std::make_shared<Constant>(0);
  auto EpsP_devForm = std::make_shared<Plas3D::Form_EpsP_devForm>(mesh);
  EpsP_devForm->eps_p=constitutive_update->eps_p();
  EpsP_devForm->f_zero=ConstZeroScalar;

  //--------------------------------------------------------------------

  auto DomainVolume = std::make_shared<Plas3D::Form_DomainVolume>(mesh);
  ...
  File file8("output/eps_v.pvd");
  File file12("output/eps_zz.pvd");
  File file9("output/epsP_v.pvd");
  File file10("output/eps_dev.pvd");
  File file11("output/epsP_dev.pvd");

  File file6("output/pdash.pvd");
  File file7("output/qdash.pvd");
  ...
  WriteToOctaveASCII("output/eps_v.txt", eps_v_global);
  WriteToOctaveASCII("output/eps_zz.txt", eps_zz_global);
  WriteToOctaveASCII("output/epsP_v.txt", epsP_v_global);
  WriteToOctaveASCII("output/eps_dev.txt", eps_dev_global);
  WriteToOctaveASCII("output/epsP_dev.txt", epsP_dev_global);
  WriteToOctaveASCII("output/pdash.txt", Pdash_global);
  WriteToOctaveASCII("output/qdash.txt", Qdash_global);
